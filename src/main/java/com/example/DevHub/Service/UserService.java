...
public class UserService {
    ...
    public User createUser(User user) {
        ...
        // ✅ If user specifies roles, allow it (e.g., ROLE_ADMIN)
        if (user.getRoles() == null || user.getRoles().isEmpty()) {
            user.setRoles(List.of("ROLE_USER")); // default role
        } else {
            // Optional: validate incoming roles (allow only ROLE_USER or ROLE_ADMIN)
            List<String> validatedRoles = new ArrayList<>();
            for (String role : user.getRoles()) {
                if (role.equals("ROLE_USER") || role.equals("ADMIN")) {
                    validatedRoles.add(role);
                }
            }

            // If invalid role provided, fallback to default
